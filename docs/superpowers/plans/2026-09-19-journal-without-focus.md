...
- [ ] **Step 5: «Закрывать после выбора» по умолчанию включено**

Решение пользователя вслед за настройкой перехвата: в `AppSettings.init` значение по умолчанию для `closeAfterSelection` — `true`. Панель закрывается после вставки, и Return сразу возвращается тексту. У тех, кто уже запускал Stash, значение записано в настройках и не меняется.
