...
- A short tour opens inside the panel on the first launch and walks through the journal in seven slides. It takes no keys at all — click the panel's left third to go back, the rest to go on, or use the button and the close cross; everything you type keeps going to the app underneath. `Tutorial` in the menu bar menu plays it again. The default theme is Stash Auto.
