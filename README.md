...
Download `Stash-<version>.zip` from [Releases](https://github.com/KiraMurano/stash/releases), unzip it and move `Stash.app` to `/Applications`. The app is not notarized, so on first launch right-click it and choose Open (or run `xattr -dr com.apple.quarantine /Applications/Stash.app`). Paste on Selection needs Accessibility access in System Settings → Privacy & Security.
